...
class TaskExecutionStatisticsReporterTest extends Specification {
    ...
    def "properly pluralizes output"() {
        ...
        then:
        TextUtil.normaliseLineSeparators(textOutputFactory as String) == "{org.gradle.internal.buildevents.BuildResultLogger}{WARN}1 actionable task: 1 executed\n"
    }
    ...
    def "reports only task counts > 0 (exec: #executed, from cache: #fromCache, up-to-date #upToDate)"() {
        ...
        then:
        TextUtil.normaliseLineSeparators(textOutputFactory as String) == "{org.gradle.internal.buildevents.BuildResultLogger}{WARN}$expected\n"
